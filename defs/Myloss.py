...
class L_con(nn.Module):
    ...
    def forward(self, x, y):
        ...
        b,c,h,w = x.shape
        x = torch.mean(x,1,keepdim=True) #(b,1,h,w)
        y = torch.mean(y,1,keepdim=True) #(b,1,h,w)
        k = torch.mean(torch.pow(abs(x - y), 1))  #number
...
class perception_loss(nn.Module):
    def __init__(self):
        ...
        features = vgg16(pretrained=True).features.cuda()  #pretrained -- saved parameters
        ...
        for x in range(4):
            self.to_relu_1_2.add_module(str(x), features[x]) #add_module(name, module) features[0] is a conv layer
        for x in range(4, 9):
